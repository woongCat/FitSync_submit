...
export interface PTScheduleContextData {
    ...
    addSchedule: (newSchedule: Omit<PTSchedule, "recordId" | "createdAt">) => Promise<boolean>;
    deleteSchedule: (recordId: string) => Promise<boolean>;
}
...
export const PTScheduleProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    // 새로운 스케줄을 추가하는 함수
    const addSchedule = async (newSchedule: Omit<PTSchedule, "recordId" | "createdAt">): Promise<boolean> => {
        try {
    ...
    // 스케줄을 삭제하는 함수
    const deleteSchedule = async (recordId: string): Promise<boolean> => {
        try {
            await axios.delete(`${Config.API_URL}/ptSchedule/${recordId}`, {
                headers: {
                    "Content-Type": "application/json",
                },
            });
            ...
            setSchedules((prevSchedules) =>
                prevSchedules.filter((schedule) => schedule.recordId !== recordId)
            );
            return true;
        } catch (error) {
